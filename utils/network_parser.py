    ...
    def _parse_edge(self, elem: ET.Element):
        ...
        if 'function' not in elem.attrib and not edge_id.startswith('-'):
            ...
            for lane in elem.findall('lane'):
                ...
                lane_length = float(lane.attrib.get('length'))
                lanes2.append({'id': lane_id, 'shape': lane_shape, 'length': lane_length})

            self.edges2[edge_id] = {'from': from_node, 'to': to_node, 'lanes': lanes2}
