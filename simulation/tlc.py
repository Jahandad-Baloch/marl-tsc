...
import numpy as np
# from data_collection import ObservationBuilder
from marl_tsc.core.obs.data_collector import DataCollector
from marl_tsc.environment.reward_machine import RewardMachine
...
class TrafficLightController:
    ...
    def detect_events(self):
        """Detect events based on traffic conditions."""
        events = []
        if self.data_collector.get_emergency_vehicle_count() > 0:
            events.append("emergency_detected")
        elif self.data_collector.get_pedestrian_button_pressed():
            events.append("pedestrian_button_pressed")
        elif self.data_collector.get_queue_length() > self.config.get("queue_threshold", 10):
            events.append("queue_threshold_exceeded")
        return events


    # Modify the pseudo_step method to handle RM logic
    def pseudo_step(self, action):
        try:
            ...
            # Emit events based on observations
            events = self.detect_events()
            for event in events:
                self.reward_machine.update_state(event)
                reward = self.reward_machine.get_reward(event)
                self.log_event({"event": event, "reward": reward})

            # Adjust duration if in the "green" phase
            if current_phase_type == 'green' and not hasattr(self, 'duration_adjusted'):
                ...
                adjusted_duration = max(
                    self.phase_min_durations[self.current_phase],
                    min(
                        self.current_phase_duration + duration_adjustment,
                        self.phase_max_durations[self.current_phase]
                    )
                )
    ...
    # Update transition_to_next_phase for RM State Tracking
    def transition_to_next_phase(self):
        ...
        # Emit queue-cleared event if appropriate
        if self.reward_machine.current_state == "queue_minimization":
            if self.data_collector.get_queue_length() <= self.config.get("queue_clear_threshold", 2):
                self.reward_machine.update_state("queue_cleared")
    ...
    def log_event(self, data):
        serializable_data = {key: float(value) if isinstance(value, (np.integer, np.floating)) else value for key, value in data.items()}
        event_data = {
            "timestamp": datetime.now().strftime("%H:%M:%S.%f"),
            "TL ID": self.id,
            "data": serializable_data,
            "RM state": self.reward_machine.current_state
        }
        self.transition_log.append(event_data)
